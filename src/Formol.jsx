...
export default class Formol extends React.PureComponent {
	...
	static getDerivedStateFromProps(nextProps, prevState) {
		...
		if (Object.entries(nextTypes).some(([ k, v ]) => v !== prevState.context.types[k])) {
	...
	async handleSubmit(e) {
		...
		const errors = (await onSubmit(transientItem, item, this.fields.names, modified)) || {};
		if (!this.mounted) {
		...
		if (
			...
			console.error(
				'onSubmit return value must be a mapping of server errors ' + "(ie: { fieldName: 'error' }) got:",
				errors
			);
			return;
		}
	...
	handleChange(name, value, error) {
		const { context: { transientItem } } = this.state;
	...
	validateForm() {
		...
		Object.entries(elements).forEach(
			([ name, { current } ]) => current && !this.errorsFromFields[name] && current.setCustomValidity('')
		);
		return Object.entries(elements).reduce((validity, [ name, { current } ]) => {
			if (validators[name]) {
				validity[name] = normalize(validators[name](transientItem[name])) || normalize(validity[name]);
			}
			...
			if (current) {
				if (current.checkValidity()) {
					...
					validity[name] =
						(validityErrors[name] && validityErrors[name](current.validity)) || current.validationMessage;
				}
			}
			return validity;
	...
	render(b) {
		...
		return (
			<form className={b.mix(className).m({ loading, errors })} onSubmit={this.handleSubmit} ref={this.form}>
				<FormolContext.Provider value={context}>{children}</FormolContext.Provider>
				{!readOnly &&
				onSubmit && (
					<React.Fragment>
						<button
